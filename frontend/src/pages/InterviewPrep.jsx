...
export default function InterviewPrep() {
  ...
  const resourcesMap = {
    ...
    "Backend Developer": [
      ...
      { title: "Backend Roadmap", link: "https://roadmap.sh/backend" },
      {
        ...
        link: "https://www.interviewbit.com/java-interview-questions/",
      },
    ],
    "Java Developer": [
  ...
  const lowestScore =
    interviewScores.length > 0 ? Math.min(...interviewScores) : 0;
    ...
    <div className="max-w-7xl mx-auto flex flex-col lg:flex-row h-[calc(100vh-4rem)] p-4 lg:p-6 gap-6 font-sans bg-slate-50/50">
      ...
      <div className="flex-1 flex flex-col bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden min-h-[600px]">
        ...
        <div
          ...
          className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 bg-[#F9FAFB] scroll-smooth"
        >
          {!isInterviewComplete && !question && !isEvaluating && !error && (
            <div className="h-full flex flex-col items-center justify-center text-center px-4 animate-fade-in">
              ...
              <p className="text-slate-500 text-sm max-w-sm">
                Configure your role and difficulty in the sidebar, then hit
                start to begin your AI-powered interview.
              </p>
            </div>
          )}
            ...
            <div className="animate-fade-in flex flex-col items-center justify-center py-10">
              <div className="w-full max-w-2xl bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm">
                ...
                <div className="bg-indigo-50/50 p-6 rounded-2xl border border-indigo-100 flex items-center justify-between mb-8">
                  <div>
                    <p className="text-[11px] font-black text-indigo-800 uppercase tracking-widest mb-1">
                      Final Readiness
                    </p>
                  ...
                  <div className="text-right">
                    <p className={`text-4xl font-black ${readinessColor}`}>
                      {readinessPercent}%
                    </p>
                  </div>
                </div>
              ...
              {feedback && (
                <div className="flex gap-3 max-w-[95%] sm:max-w-[85%]">
                  ...
                  <div className="bg-white border border-slate-200 rounded-2xl rounded-tl-sm shadow-sm w-full overflow-hidden">
                    <div
                      className="p-4 flex items-center justify-between bg-slate-50/50 cursor-pointer hover:bg-slate-50 transition-colors"
                      onClick={() => setIsFeedbackExpanded(!isFeedbackExpanded)}
                    >
                      <div className="flex items-center gap-3">
                        <span
                          className={`px-3 py-1 rounded-md text-xs font-black border ${scoreBadgeColor}`}
                        >
            ...
            <div className="flex flex-col gap-3">
              <div className="relative">
                <textarea
                  ...
                  placeholder={
                    !question
                      ? "Start a session to enable typing..."
                      : feedback
                        ? "Review feedback, then click Next Question."
          ...
          {interviewScores.length > 0 ? (
              ...
              <div className="flex items-center justify-between mb-6 pb-6 border-b border-slate-100">
                <div>
                  ...
                  <div className="flex items-end gap-1">
                    <span
                      className={`text-4xl font-black leading-none tracking-tight ${readinessColor}`}
                    >
                      {readinessPercent}%
                    </span>
                  </div>
                ...
                <div
                  className={`px-3 py-1.5 rounded-md text-xs font-bold border ${scoreBadgeColor}`}
                >
                  {readinessLabel}
                </div>
              </div>
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">
                      {Array.from(new Set(allStrengths)).length}
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {allStrengths.length > 0 ? (
                      Array.from(new Set(allStrengths))
                        .slice(0, 3)
                        .map((str, idx) => (
                          <span
                            key={`str-${idx}`}
                            className="px-2 py-1 bg-emerald-50 text-emerald-700 rounded text-[10px] font-bold border border-emerald-100 truncate max-w-full"
                          >
                            {str}
                          </span>
                        ))
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-xs font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-full">
                      {Array.from(new Set(allWeaknesses)).length}
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {allWeaknesses.length > 0 ? (
                      Array.from(new Set(allWeaknesses))
                        .slice(0, 3)
                        .map((wk, idx) => (
                          <span
                            key={`wk-${idx}`}
                            className="px-2 py-1 bg-red-50 text-red-700 rounded text-[10px] font-bold border border-red-100 truncate max-w-full"
                          >
                            {wk}
                          </span>
                        ))
            ...
            <div className="text-center py-8 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
              ...
              <p className="text-xs text-slate-500 font-bold px-4">
                Insights will appear here after your first answer.
              </p>
            </div>
          )}
        </div>
